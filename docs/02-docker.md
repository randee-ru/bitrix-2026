...
2. В том же файле заданы переопределения:
   ...
   - **MySQL** проброшен на хост **`127.0.0.1:33061`** по умолчанию (переменная **`HOST_MYSQL_PORT`** в корневом `.env`, если порт занят).
...
- С **хоста** (клиенты MySQL): `127.0.0.1:${HOST_MYSQL_PORT:-33061}`.
